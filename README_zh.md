...
> 毫秒级免重登切号，全账号模型配额大盘，聊天记录与插件零污染。

---

## ✨ 为什么选择 agy-orbit？

在日常高强度使用 Google Antigravity CLI (`agy`) 编程时，经常会遇到 **429 配额用尽** 的尴尬，而官方目前尚未原生支持多账号切换。如果手动重新登录，不仅要反复弹浏览器走网页 OAuth 授权流程，粗暴地复制或替换配置目录更极易搞丢宝贵的本地会话历史（`brain/`）。

`agy-orbit` (`agyo`) 为解决这些痛点而生：

- ⚡ **一秒免重登切号**：保存多个账号快照后，只需 `agyo use <name>` 即可在毫秒级内完成切换（无需弹浏览器重新登录）；直接敲 `agyo` 更能呼出方向键交互菜单，上下选中回车即切。
